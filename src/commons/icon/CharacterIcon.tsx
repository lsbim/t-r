...
const CharacterIcon = ({
    ...
    const iconConfig = type === 'small'
        ...
        : type === 'mini'
            ? `w-[52px] z-20 h-[60px] border border-black flex flex-col justify-center items-center ${personality === '공명' ? 'resonance-pers' : `bg-${personality}`}`
            : type === 'micro'
    ...
    const nameConfig = type === 'small'
        ...
        : type === 'mini'
            ? "bg-white bg-opacity-80 font-bold w-full px-[2px] truncate text-[11px] min-h-[16px] z-10 flex items-center justify-center"
            : type === 'micro'
                ? "hidden"
    ...
    return (
        <div
            ...
            >
                <img
                    className={`${type === 'mini' ? 'w-full object-cover object-top origin-top scale-[1.3]' : 'w-full'} ${type === 'micro' ? 'object-cover' : 'object-contain'}`}
                    src={imgURL(name)}
                    decoding="async" />
            </div>
