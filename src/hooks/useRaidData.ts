...
import { FrontierExternalData, FrontierPlayerData, FrontierSeasonData, FrontierSummary } from "../types/frontierTypes";
import { ClashExternalData, ClashPlayerData, ClashSeasonData, ClashSummary } from "../types/clashTypes";
import { ClashV2SeasonData, ClashV2Summary } from "../types/clashV2Types";
...
const fetchSummaryData = async (type: TrickcalRaidEn | 'latest', category: 'summary' | 'stats') => {
    ...
    let response = category === 'summary' ? await fetch(`/data/${typeDir}/summaries.json`)
        : category === 'stats' ? await fetch(`/data/${typeDir}/personality.json`)
            : undefined;
...
export const useRaidData =
    <T extends FrontierSeasonData
        ...
        return useQuery<T, Error>({
            queryKey: [type, category, season],
            queryFn: async () => {


                // 시즌 데이터를 불러올 경우
                if (season && type !== 'latest') {
                    ...
                    return await fetchSummaryData(type, category);
                }
            },
